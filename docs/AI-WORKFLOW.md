...
`submit_edit_plan` 是一个**返回数据而非副作用**的虚拟工具：

- 模型必须调用它（`tool_choice: { type: 'tool', name: 'submit_edit_plan' }`），禁止直接文本回复
- `input_schema` 是从 `PlanPayloadSchema` 通过 `z.toJSONSchema` 派生的完整 JSON Schema（Draft 2020-12）
- 工具参数即为候选计划，节点格式校验交给浏览器 reducer 执行
- 切换 provider 时，无需修改任何契约字段
...
Vercel 网关端到端期限为 90 秒，每次上游尝试最多 75 秒。这样在模型较慢时仍能返回首个结果，并为一次格式修复或安全错误响应保留时间。SDK 自动重试设为 0。网络超时、429 和上游 5xx 都不自动重发，界面提供重新请求入口，并告知原请求可能已经产生费用。
...
应用日志不保存图片 base64、用户输入、文件名、EXIF、Cookie 内容或密钥。为诊断模型兼容性，运行日志会记录模型返回的 JSON、请求随机 ID、状态码、耗时、模型版本和用量。日志保留时间由 Vercel 或 Cloudflare 的项目设置决定。
