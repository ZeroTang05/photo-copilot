...
日期：2026-09-23。状态：实施设计，尚未进行新提示词的真实效果评测。配套 [阶段提示词](AI-COLOR-PROMPTS-V2.md)与[照片内容及光线模块](AI-COLOR-SCENE-MODULES.md)。

## 当前接入状态

已接入共享能力表、版本化阶段 Prompt、工作流快照与 R→D→P→V→C 的有界编排。D 同时生成 SceneProfile；P 和 V 组合对应的主体、光线模块。浏览器真实渲染候选后进入 V，最多执行一次 C 并复查。明确的局部目标复用已有选区；D 提出新 SAM 概念时，当前界面提示用户先建立选区。完整自动 T 编排、局部图册与真实照片对照评测仍待实施。

## 1. 目标与设计依据

目标是让模型稳定理解用户审美、正确使用本项目参数，并根据真实预览判断结果。质量由四部分共同决定：图像信息、渲染器能力说明、阶段任务划分、真实照片评测。

采用按任务分流的固定工作流；综合任务在诊断阶段判断照片主体与光线条件，再组合对应的提示词模块。各阶段可以使用同一个模型，但使用独立输入和输出契约。只有需要的阶段才调用；代码负责路由权限、资源身份、参数合法性和像素执行。
...
D 同时生成 SceneProfile：最多两个与当前目标相关的照片主体、最多两个光线标签、可见证据与主要主体。程序用这个结果从[模块库](AI-COLOR-SCENE-MODULES.md)选取提示词。人像、风景、建筑等主体可以并存；夜景、逆光、混合光等条件独立组合。不会为分类单独加一次 LLM 调用。
...
主体类型决定默认检查重点，用户意图决定调整方向与范围。例如人物站在晚霞海边：用户说“自然优化”时同时关注人物与天空；用户说“只让人物亮一点”时，局部范围锁定人物选区，保留晚霞。
...
D 的一次输出同时包含 EditBrief 与 sceneProfile。sceneProfile 只影响 P、V 的关注点模块；已有用户锁定项、权限与能力表保持权威。quick 路线直接进入 P，使用公共规则和任务模块，必要的图像判断在本次规划中完成，无额外分类调用。用户手动更改照片或裁切使主体离开画面时，重算相关诊断。
...
| SceneProfile | 在 D 的同一次输出中提供；主体最多 2 项、光线标签最多 2 项、主要主体与可见依据；枚举和选择规则见模块库 |
...
建议在共享提示词包中维护 common、router、brief、planner、reviewer、corrector、任务片段、主体片段及光线片段。每个模块独立版本管理，两个网关共享。程序按 D 的 SceneProfile 显式选择最多两个主体、两个光线模块。公共能力卡独立版本管理。Schema 从同一 Zod 定义生成，禁止提示词复制一套不同参数表。
...
3. **W3 综合路线。** 加入 D 中的 SceneProfile、主体与光线模块、参考风格与恢复任务片段，统一 SAM 概念发现；保留按任务短路。
4. **W4 视觉闭环。** 真实渲染后调用 V，加入一次 C 与复查、预算和过期响应处理。
5. **W5 真实比较。** 新旧工作流同图同意图比较，通过门槛再默认启用新流程。

## 12. 评测与上线门槛

准备至少 40 张真实照片，覆盖人像、夜景、逆光、风景、建筑、商品、宠物、室内混光、高低调、混合主体及细小选区；扩展为至少 80 条真实任务。固定留出 20% 任务不用于调提示词。同一场景尽量使用不同照片做训练示例与验收，避免记住样本。
...
| 主体与光线模块 | 同一请求在人像、风景等不同照片上的关注点符合可见内容；混合场景保留两个主体；无越权编辑 |
