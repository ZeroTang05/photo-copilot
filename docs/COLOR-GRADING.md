...
| 局部蒙版 | Brush / Radial / Graduated | Layers + 多种 Mask | Power Window | **v1 仅椭圆**(已有);**v2 加 画笔/渐变/径向** |
...
**结论**:v1 仅启用椭圆(现有);画笔/渐变/径向作为占位控件;v2 优先加**径向**(对应 Lightroom 径向滤镜,效果立竿见影)。
...
| 占位名 | 含义 | 真实名称 | v2 落地路径 |
|---|---|---|---|
| 自然饱和度 | Vibrance | (已在 v1 启用,见 §4.1) | — |
| 白色色阶 | Whites | (已在 v1 启用) | — |
| 黑色色阶 | Blacks | (已在 v1 启用) | — |
| 画笔 | Adjustment Brush | v2 局部蒙版工具 | 引入蒙版 alpha 字段到 `RegionSchema`,默认保留椭圆退化 |
| 渐变 | Graduated Filter | v2 局部蒙版工具 | 同上 |
| 径向 | Radial Filter | v2 局部蒙版工具 | 同上 |

注:`自然饱和度`/`白色色阶`/`黑色色阶` 在 v1 中已经升级为真实控件,占位控件就删除。`画笔`/`渐变`/`径向` 在 v1 仍为占位。
...
按 §4.1 的全局参数表组织 11 个 Slider;移除"白色色阶 / 黑色色阶 / 自然饱和度" 3 个占位控件;`画笔 / 渐变 / 径向` 3 个仍为占位。

视觉组织:
- **构图**(保留):裁剪比例 / 旋转 / 4 个裁切 / 允许 AI 构图 勾选。
- **光线**:曝光 / 对比 / 高光 / 阴影 / 白色色阶 / 黑色色阶 / 清晰度。
- **色彩**:色温 / 色调 / 自然饱和度 / 饱和度。
- **局部调整**(保留):椭圆添加按钮 / 画笔 / 渐变 / 径向(占位)。
