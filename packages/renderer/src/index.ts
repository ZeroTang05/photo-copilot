...
uniform float u_exposure,u_contrast,u_highlights,u_shadows,u_warmth,u_tint,u_saturation,u_angle;
...
void main(){
  ...
  base*=exp2(u_exposure); float y=clamp(lum(base),0.,1.); base*=exp2(u_shadows*(1.-smoothstep(.05,.5,y))+u_highlights*smoothstep(.45,.95,y)); base=sat(base,u_saturation);
